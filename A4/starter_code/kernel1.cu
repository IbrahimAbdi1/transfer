...
void run_kernel1(const int8_t *filter, int32_t dimension, const int32_t *input,
  ...
  printf("soring pixelCount %d numBlocks %d\n",pixelCount,numBlocks);
...
__global__ void find_min_max(int32_t *arr,int32_t *max_min,int32_t pixelCount,int pixelBlock){
    ...
    // complete unroll 
        ...
        if(threadID == 0){
            ...
            max_min[blockIdx.x*2] = (int32_t)max_min_data[0][0];

            max_min[blockIdx.x*2+1] = (int32_t)max_min_data[1][0];



            printf("max %d stored in %d min %d stored in %d\n", max_min[blockIdx.x*2],blockIdx.x*2,max_min[blockIdx.x*2+1],blockIdx.x*2+1);
