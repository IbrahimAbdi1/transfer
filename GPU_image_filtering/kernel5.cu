...
void run_kernel5(const int8_t *filter, int32_t dimension, const int32_t *input, int32_t *output, int32_t width, int32_t height, int32_t *g_min_max)
{
  ...
  kernel5<<<numBlocks + 1, 1024>>>(dimension, input, output, width, height);
  ...
  normalize5<<<numBlocks + 1, 1024>>>(output, width, height, min, max);
}
